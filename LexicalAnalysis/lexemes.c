...
EndState* pointer;
EndState* end_states_arr[END_STATE_SIZE];
...
int FillEndStatesArr() {
...

	{
		...
		if (end_states_arr[x] != NULL)
		{
			...
			end_states_arr[x]->next = malloc(sizeof(EndState));
...
int findToken(int* state, char* word)
{
	...
	while (end_states_arr[i]->next != NULL)
	{
		...
		if (x == *state)
		{
			ListOfTokens(word, end_states_arr[i]->nameToken, end_states_arr[i]->num);

			return 1;
		}
	}
	/*for (int i = 0; i < END_STATE_SIZE; i++)
	{
		x= end_states_arr[i]->numToken;
		if(x==*state){
			ListOfTokens(word, end_states_arr[i]->nameToken, end_states_arr[i]->num);
