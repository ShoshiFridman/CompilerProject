...
void Grammar()
{
    fileGrammar = fopen("C:/Users/User/Desktop/compilerProject/CFG.txt", "r");
    if (fileGrammar == NULL)
    {
    ...
    while (fgets(buffer, sizeof(buffer), fileGrammar) != EOF) {
        ...
        tokens[num_tokens++] = start;
        _itoa(num_tokens, matrix[row][NO_ERROR], NUM);//הכנסת מס' המחרוזות בשורה לאינדקס הראשון במטריצה
